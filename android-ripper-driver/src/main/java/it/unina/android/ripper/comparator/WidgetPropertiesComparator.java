...
public class WidgetPropertiesComparator implements IComparator, Serializable {
	 ...
	 * Constructor
	 */
	public WidgetPropertiesComparator()
	{
		super();
	}
	...
	protected boolean matchWidget(WidgetDescription w1, WidgetDescription w2)
	{
		...
		if (w1.getName().equals(w2.getName()) == false) {
			debug("compare widget name("+w1.getName()+","+w2.getName()+") -> false");
			return false;
		} else {
			debug("compare widget name("+w1.getValue()+","+w2.getValue()+") -> true");	
		}
		
		if (w1.getValue().equals(w2.getValue()) == false) {
			debug("compare widget values("+w1.getValue()+","+w2.getValue()+") -> false");
			return false;
		} else {
			debug("compare widget values("+w1.getValue()+","+w2.getValue()+") -> true");	
		}	
